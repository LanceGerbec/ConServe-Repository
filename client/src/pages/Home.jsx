    ...
    <div className="relative w-full overflow-hidden" style={{ height: '560px' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 flex items-center h-full pb-16">
        <div className="max-w-xl" style={{ opacity: fading ? 0 : 1, transition: 'opacity 0.5s ease 0.1s' }}>
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white uppercase leading-tight mb-5 font-sans" style={{ letterSpacing: '-0.02em' }}>
            {slide.headline || 'DISCOVER\nNURSING\nRESEARCH'}
          </h1>
          <p className="text-sm md:text-base text-white/70 mb-8 leading-relaxed max-w-sm font-sans">
            {slide.sub || 'Access peer-reviewed nursing papers from NEUST. Collaborate, submit, and cite with ease.'}
          </p>
          <Link to="/explore"
            className="inline-flex items-center gap-2 px-8 py-3.5 border-2 border-white text-white font-bold uppercase text-sm tracking-widest hover:bg-white hover:text-[#0d1f3c] transition-all duration-200 font-sans">
            EXPLORE →
          </Link>
        </div>
      </div>
...
/* ── Proportional Image Card for About/Research sections ── */
const ProportionalCard = ({ url, alt = '', className = '' }) => (
...
export default function Home() {
  ...
  // Separate home page images (about section + research type cards)
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/settings`).then(r => r.json()).then(d => {
      ...
      // Hero slideshow: heroBg + bannerImages
      const imgs = [];
      ...
      // Home page section images (separate from banners)
      if (s.homeImages) {
        setHomeImages({
          about: s.homeImages.about || ['', '', ''],
          types: s.homeImages.types || ['', '', '']
        });
      } else {
        // Fallback: use bannerImages for these sections too
        const bi = (s.bannerImages || []).map(b => b.url);
        setHomeImages({
          about: [bi[0] || '', bi[1] || '', bi[2] || ''],
          types: [bi[0] || '', bi[1] || '', bi[2] || '']
        });
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 overflow-x-hidden font-sans">
      ...
      {/* ══ ABOUT + PROPORTIONAL IMAGES ══ */}
      <section className="bg-gray-50 dark:bg-gray-950 py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          {/* LEFT: proportional image grid */}
          <div className="grid grid-cols-2 gap-3">
            {/* Top full-width */}
            <div className="col-span-2">
              <ProportionalCard url={homeImages.about[0]} alt="About 1" className="w-full rounded-xl shadow-xl" />
            </div>
            {/* Bottom two equal */}
            <div>
              <div className="relative">
                ...
                <ProportionalCard url={homeImages.about[1]} alt="About 2" className="w-full rounded-xl shadow-lg mt-6" />
              </div>
            </div>
            <div>
              <div className="relative">
                <ProportionalCard url={homeImages.about[2]} alt="About 3" className="w-full rounded-xl shadow-lg" />
          ...
          {/* RIGHT: text */}
          <div>
            <span className="inline-block px-4 py-1.5 rounded-full border border-gray-300 dark:border-gray-600 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-6 font-sans">
              ABOUT US
            </span>
            <h2 className="text-3xl md:text-4xl font-black text-[#0d1f3c] dark:text-white uppercase leading-tight mb-5 font-sans" style={{ letterSpacing: '-0.02em' }}>
              FIND AND EXPLORE<br />THE RESEARCH<br />OF YOUR DREAMS
            </h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-6 max-w-sm font-sans">
            ...
            <Link to="/explore"
              className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#0d1f3c] dark:bg-blue-600 text-white font-bold uppercase text-sm tracking-widest hover:bg-blue-900 dark:hover:bg-blue-700 transition-all font-sans">
              EXPLORE →
            </Link>
          </div>
        </div>
      ...
      {/* ══ SERVICES — dark navy, mobile horizontal scroll ══ */}
      ...
      {/* ══ RESEARCH TYPES — proportional, mobile horizontal scroll ══ */}
      <section className="bg-gray-50 dark:bg-gray-950 py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
            <div>
              <span className="inline-block px-4 py-1.5 rounded-full border border-gray-300 dark:border-gray-600 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-4 font-sans">
                RESEARCH TYPES
              </span>
              <h2 className="text-2xl md:text-4xl font-black text-[#0d1f3c] dark:text-white uppercase leading-tight font-sans" style={{ letterSpacing: '-0.02em' }}>
                WHICH RESEARCH<br />TYPE SUITS<br />YOU BEST?
              </h2>
            </div>
            <Link to="/explore"
              className="self-start inline-flex items-center gap-2 px-6 py-3 bg-[#0d1f3c] dark:bg-blue-600 text-white font-bold uppercase text-sm tracking-widest hover:bg-blue-900 dark:hover:bg-blue-700 transition-all whitespace-nowrap font-sans">
              SHOW MORE →
            </Link>
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 md:pb-0 md:grid md:grid-cols-3 md:gap-1 scrollbar-hide snap-x snap-mandatory">
            {TYPES.map((t, i) => (
              <Link to="/explore" key={i}
                className="group relative overflow-hidden bg-[#0d1f3c] block flex-shrink-0 w-72 md:w-auto snap-start rounded-xl md:rounded-none"
              ...
              >
                ...
                <div className="absolute top-4 right-4 w-8 h-8 bg-white rounded-full flex items-center justify-center group-hover:bg-blue-600 transition-colors z-10">
                  <ArrowRight size={14} className="text-[#0d1f3c] group-hover:text-white transition-colors" />
                </div>
      ...
      <section className="bg-[#0d1f3c] py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h2 className="text-2xl md:text-4xl font-black text-white uppercase leading-tight font-sans" style={{ letterSpacing: '-0.02em' }}>
              START YOUR RESEARCH<br />JOURNEY TODAY
            </h2>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <Link to="/explore"
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 border-2 border-white text-white font-bold uppercase text-sm tracking-widest hover:bg-white hover:text-[#0d1f3c] transition-all font-sans">
              BROWSE PAPERS →
            </Link>
            {!user && (
              <Link to="/register"
                className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-blue-600 text-white font-bold uppercase text-sm tracking-widest hover:bg-blue-500 transition-all font-sans">
                REGISTER FREE →
              </Link>
            )}
