// client/src/pages/Explore.jsx - Enhanced Mobile-Responsive Version
import { useState, useEffect, useCallback, useMemo, memo, useRef } from 'react';
import { Search, Filter, X, Eye, Calendar, BookOpen, SlidersHorizontal, Sparkles, Info, TrendingUp, Lightbulb, Grid, List, Award, ChevronLeft, ChevronRight, Play, Pause, Star, Flame, Plus, Edit2, Trash2, Save, Crown, ArrowUp } from 'lucide-react';
...
// ── Featured Papers Horizontal Scroll (mobile-optimized) ──
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center gap-2 mb-2 px-0.5">
        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-yellow-50 dark:bg-yellow-900/20 rounded-full border border-yellow-300 dark:border-yellow-700">
          <Crown size={12} className="text-yellow-600 dark:text-yellow-400" />
          <span className="text-xs font-bold text-yellow-700 dark:text-yellow-300 uppercase tracking-wide">Featured</span>
        </div>
      </div>
      {/* Horizontal scroll on mobile, wrap on desktop */}
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x snap-mandatory" ref={trackRef}>
        {papers.map((item, i) => {
          const colors = getSubjectColor(item.paper?.subjectArea || item.subjectArea);
          const paper = item.paper || item;
          const caption = item.caption;
          return (
            <div key={`${paper._id}-${i}`} onClick={() => onPaperClick(paper._id)}
              className={`flex-shrink-0 snap-start w-[220px] sm:w-[240px] bg-white dark:bg-gray-800 rounded-xl border-l-4 ${colors.card} shadow cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200`}
              style={{ border: '1px solid rgba(0,0,0,0.07)', borderLeft: undefined }}>
              <div className={`rounded-xl border-l-4 ${colors.card} h-full`}>
                <div className="p-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs font-bold border ${colors.bg} ${colors.border} ${colors.text}`}>
                  ...
                  <h3 className="font-bold text-xs text-gray-900 dark:text-white line-clamp-2 mb-1 leading-snug">{paper.title}</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mb-2">{paper.abstract}</p>
                  {caption && <p className="text-xs text-yellow-700 dark:text-yellow-400 italic line-clamp-1 mb-1"><Star size={9} className="inline mr-0.5" />{caption}</p>}
                  <div className="flex items-center justify-between mt-1 pt-1.5 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-xs text-gray-400 truncate">{paper.authors?.[0]}{paper.authors?.length > 1 ? ` +${paper.authors.length - 1}` : ''}</p>
...
const FeaturedPapersManager = memo(({ allPapers, featuredList, onSave, onClose }) => {
  ...
  const toggle = (paper) => {
    ...
    else if (selected.length < 8) setSelected(prev => [...prev, { paperId: paper._id, caption: '' }]);
  };
    ...
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2"><Crown size={18} className="text-yellow-600" /><h3 className="font-bold text-gray-900 dark:text-white">Featured Papers ({selected.length}/8)</h3></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <div>
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto">
              {filtered.map(p => (
                <div key={p._id} onClick={() => toggle(p)} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition ${isSelected(p._id) ? 'bg-navy/10 dark:bg-navy/20 border border-navy/30' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
...
const PaperCard = memo(({ paper, onClick, highlight, viewMode }) => {
  ...
  if (viewMode === 'list') {
    return (
      <div onClick={() => onClick(paper._id)} className={`group bg-white dark:bg-gray-800 rounded-xl border-l-4 ${colors.card} border border-gray-200 dark:border-gray-700/60 p-3 cursor-pointer hover:shadow-md transition-all duration-150 hover:-translate-y-0.5`}>
        <div className="flex gap-2.5">
          <div className="flex-shrink-0 w-9 h-9 bg-navy/8 dark:bg-navy/20 rounded-lg flex items-center justify-center mt-0.5">
            <BookOpen size={14} className="text-navy dark:text-accent" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-1.5 mb-0.5">
              <h3 className="font-bold text-xs text-gray-900 dark:text-gray-100 line-clamp-1 group-hover:text-navy dark:group-hover:text-accent flex-1">{hl(paper.title)}</h3>
              <span className="px-1.5 py-0.5 bg-navy/10 text-navy dark:bg-accent/20 dark:text-accent rounded text-xs font-semibold whitespace-nowrap flex-shrink-0">{paper.category}</span>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mb-1">{hl(paper.abstract)}</p>
  ...
  // Grid card - compact
  return (
    <div onClick={() => onClick(paper._id)} className={`group bg-white dark:bg-gray-800 rounded-xl border-l-4 ${colors.card} border border-gray-200 dark:border-gray-700/60 p-3 cursor-pointer hover:shadow-lg transition-all duration-150 hover:-translate-y-0.5 flex flex-col`}>
      <div className="flex items-start justify-between mb-2">
        <span className="px-2 py-0.5 bg-navy/10 text-navy dark:bg-accent/20 dark:text-accent rounded text-xs font-bold">{paper.category}</span>
        <span className="flex items-center gap-0.5 text-xs text-gray-400"><Eye size={10} />{paper.views || 0}</span>
      </div>
      <h3 className="font-bold text-xs text-gray-900 dark:text-gray-100 line-clamp-2 mb-1.5 group-hover:text-navy dark:group-hover:text-accent leading-snug">{hl(paper.title)}</h3>
      <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 flex-1 mb-2">{hl(paper.abstract)}</p>
...
// ── Floating Quick Search Bar (appears on scroll) ──
...
const Explore = () => {
  ...
  // Floating search visibility
  useEffect(() => {
    const handleScroll = () => {
      const y = window.scrollY;
      setScrollY(y);
      setShowFloatingSearch(y > 300);
    };
  ...
  const performSearch = async (overrideQuery) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const params = new URLSearchParams({ status: 'approved', limit: 1000, ...(q && { [searchMode === 'advanced' ? 'query' : 'search']: q }), ...(filters.category && { category: filters.category }), ...(filters.yearCompleted && { yearCompleted: filters.yearCompleted }), ...(filters.subjectArea && { subjectArea: filters.subjectArea }), ...(filters.author && { author: filters.author }), ...(searchMode === 'advanced' && semantic && { semantic: 'true' }) });
  ...
  return (
    <>
      ...
      <FloatingSearch onSearch={handleFloatingSearch} visible={showFloatingSearch} />
      ...
      <div className="min-h-screen pb-8 bg-gray-50 dark:bg-gray-950">
        ...
        {/* ── Featured Papers - Horizontal Scroll ── */}
        <div className="mb-3">
          {isAdmin && (
            <div className="flex justify-end mb-1">
              <button onClick={() => setShowFeaturedManager(true)} className="flex items-center gap-1 px-2.5 py-1 bg-yellow-400/20 hover:bg-yellow-400/30 border border-yellow-400/40 rounded-lg text-xs font-bold text-yellow-700 dark:text-yellow-300 transition">
                <Edit2 size={11} />Manage Featured
              </button>
            </div>
          )}
          {resolvedFeatured.length > 0 ? (
            ...
            <div className="border-2 border-dashed border-yellow-300 dark:border-yellow-700 rounded-xl p-5 text-center bg-yellow-50/50 dark:bg-yellow-900/10">
              <Crown size={24} className="mx-auto text-yellow-400 mb-1.5" />
              <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2">No featured papers yet</p>
              <button onClick={() => setShowFeaturedManager(true)} className="flex items-center gap-1.5 mx-auto px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white rounded-xl font-bold text-xs transition"><Plus size={12} />Add Featured</button>
            </div>
          ) : null}
        </div>

        {/* ── Search Panel ── */}
        <div ref={searchPanelRef} className="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-3 mb-4">
          {/* Mode Toggle */}
          <div className="grid grid-cols-2 gap-1.5 mb-2.5">
            <button onClick={() => setSearchMode('simple')} className={`px-3 py-2 rounded-xl font-bold text-xs transition flex items-center justify-center gap-1.5 ${searchMode === 'simple' ? 'bg-navy text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600'}`}><Search size={13} />Simple</button>
            <button onClick={() => setSearchMode('advanced')} className={`px-3 py-2 rounded-xl font-bold text-xs transition flex items-center justify-center gap-1.5 ${searchMode === 'advanced' ? 'bg-purple-600 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600'}`}><Sparkles size={13} />Advanced</button>
          </div>
          {/* Search Input */}
            ...
            <div className="relative">
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-navy/20 dark:border-accent/30 rounded-xl shadow-xl z-20 overflow-hidden">
                  {suggestions.map((s, i) => (<button key={i} type="button" onClick={() => { setQuery(s.text); setShowSuggestions(false); setTimeout(() => performSearch(s.text), 50); }} className="w-full text-left px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center justify-between gap-2 transition text-xs">
                    <span className="text-gray-900 dark:text-gray-100 line-clamp-1 flex-1">{s.text}</span>
                    <span className="text-gray-400 capitalize flex-shrink-0">{s.type}</span>
                  </button>))}
                </div>
              )}
            </div>
        ...
        {!activeQuery && recommendations.length > 0 && (
          <div className="mb-4 p-3 bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-xl border border-purple-100 dark:border-purple-800">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wide">Recommended</span>
            </div>
        ...
        <div ref={resultsRef}>
          ...
          <div className="flex items-center justify-between mb-2 gap-2">
            <p className="text-xs text-gray-600 dark:text-gray-300">
              <strong className="text-navy dark:text-accent text-sm">{sortedPapers.length}</strong> papers
              {activeCount > 0 && <button onClick={clearAll} className="ml-1.5 text-xs text-red-500 font-bold inline-flex items-center gap-0.5"><X size={10} />clear</button>}
            </p>
          ...
          {/* Subject Legend - compact horizontal scroll */}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 mb-3">Try different keywords</p>
              {activeCount > 0 && <button onClick={clearAll} className="px-4 py-2 bg-navy text-white rounded-xl font-bold text-xs">Show All</button>}
            </div>
          ) : (
            <>
      ...
      {showTips && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowTips(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-5 border border-navy/20" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-xs text-gray-700 dark:text-gray-300">
              <div className="p-2.5 bg-blue-50 dark:bg-blue-900/30 rounded-xl border border-blue-200 dark:border-blue-700">
                <p className="font-bold mb-1">Simple Search</p>
              ...
              <div className="p-2.5 bg-purple-50 dark:bg-purple-900/30 rounded-xl border border-purple-200 dark:border-purple-700">
                <p className="font-bold mb-1">Advanced / Boolean</p>
                <p>diabetes AND management · pediatric OR children · author:Reyes · year:2024</p>
              </div>
              <div className="p-2.5 bg-yellow-50 dark:bg-yellow-900/30 rounded-xl border border-yellow-200 dark:border-yellow-700">
                <p className="font-bold mb-1">Color-coded cards</p>
              ...
              <div className="p-2.5 bg-green-50 dark:bg-green-900/30 rounded-xl border border-green-200 dark:border-green-700">
                <p className="font-bold mb-1">Quick Search</p>
                <p>Scroll down to see a floating search bar for quick access anywhere on the page.</p>
              </div>
            </div>
          </div>
        </div>
