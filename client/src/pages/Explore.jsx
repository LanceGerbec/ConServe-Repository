...
import {
  ...
  Info, TrendingUp, Grid, List, Award, ChevronLeft, ChevronRight, Play, Pause,
  Plus, Edit2, Save, Crown, Cpu, HelpCircle
} from 'lucide-react';
    ...
    <div className="relative inline-flex items-center" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      ...
      {show && (
        <div className={`absolute left-1/2 -translate-x-1/2 ${posClass} z-50 w-64 bg-gray-900 dark:bg-gray-950 text-white text-xs rounded-xl shadow-2xl p-3 border border-gray-700 pointer-events-none`}>
          {content}
          <div className={`absolute left-1/2 -translate-x-1/2 ${position === 'bottom' ? 'bottom-full border-b-gray-900 dark:border-b-gray-950' : 'top-full border-t-gray-900 dark:border-t-gray-950'} border-4 border-transparent`} />
        </div>
      )}
    </div>
  );
...
const AwardBadge = memo(({ award, small }) => {
  ...
  return (
    <div className="relative inline-block" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      ...
      {show && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg shadow-xl whitespace-nowrap z-50 border border-gray-700">
          {award.name}
...
const HeroBanner = memo(({ bannerImages }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-2xl" style={{ height: 'clamp(160px, 32vw, 340px)' }}>
      <div className="absolute inset-0 bg-cover bg-center transition-all duration-700"
        style={{ backgroundImage: `url(${img.url})`, opacity: fading ? 0 : 1, transform: fading ? 'scale(1.03)' : 'scale(1)' }} />
      ...
      {img.caption && (
        <div className="absolute bottom-10 left-5 right-16 md:left-8">
          <p className="text-white font-bold text-sm md:text-base lg:text-lg drop-shadow-lg max-w-xl"
            style={{ opacity: fading ? 0 : 1, transition: 'opacity 0.5s ease 0.1s' }}>{img.caption}</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
            {bannerImages.map((_, i) => (
              <button key={i} onClick={() => goTo(i)} className={`rounded-full transition-all duration-300 ${i === current ? 'w-5 h-2 bg-white' : 'w-2 h-2 bg-white/50 hover:bg-white/80'}`} />
            ))}
          </div>
          {!paused && (
...
// ─── Featured Papers Floating Cards ──────────────────────────────────────────
    ...
    <div className="relative">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-amber-400/20 to-yellow-400/20 dark:from-amber-500/20 dark:to-yellow-500/20 rounded-full border border-amber-300/50 dark:border-amber-600/50 shadow-sm">
        ...
        {isAdmin && (
          <button onClick={onManage} className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/20 dark:hover:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-xl text-xs font-bold text-amber-700 dark:text-amber-300 transition">
      ...
      {papers.length === 0 && isAdmin ? (
        <div className="border-2 border-dashed border-amber-300 dark:border-amber-700 rounded-2xl p-10 text-center bg-amber-50/30 dark:bg-amber-900/10">
          ...
          <p className="text-xs text-gray-400 mb-4">Select papers to feature them for all users</p>
          <button onClick={onManage} className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold text-sm transition shadow-md">
            <Plus size={14} />Add Featured Papers
          </button>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {papers.map((item, i) => {
              ...
              const floatDuration = `${3.5 + (i % 3) * 0.6}s`;
              return (
                <div
                  key={paper._id}
                  ...
                  style={{
                    animation: isHovered ? 'none' : `float-card ${floatDuration} ease-in-out ${animDelay} infinite`,
                  }}
                >
                  {/* Glow effect */}
                  <div className="absolute -inset-0.5 rounded-2xl opacity-0 group-hover:opacity-60 transition-opacity duration-300 blur-sm"
                    style={{ background: `linear-gradient(135deg, ${colors.accent}40, ${colors.accent}20)` }} />

                  <div className={`relative bg-white dark:bg-gray-800 rounded-2xl border-l-4 shadow-lg group-hover:shadow-2xl transition-all duration-300 overflow-hidden
                    ${isHovered ? '-translate-y-2 scale-[1.02]' : ''}`}
                    style={{ borderLeftColor: colors.accent, border: '1px solid rgba(0,0,0,0.06)', borderLeft: `4px solid ${colors.accent}` }}
                  >
                    {/* Top accent bar */}
                    ...
                    <div className="p-4">
                      ...
                      {/* Title */}
                      <h3 className="font-bold text-sm text-gray-900 dark:text-white line-clamp-2 mb-2 leading-snug group-hover:text-navy dark:group-hover:text-accent transition-colors">
                        {paper.title}
                      </h3>
...
// ─── Featured Papers Manager (Admin Modal) ────────────────────────────────────
const FeaturedPapersManager = memo(({ allPapers, featuredList, onSave, onClose }) => {
  const [selected, setSelected] = useState(
    featuredList.map(f => ({ paperId: f.paperId || f.paper?._id || f._id, caption: f.caption || '' }))
  );
  ...
  const toggle = (paper) => {
    ...
    else { if (selected.length >= 8) return; setSelected(prev => [...prev, { paperId: paper._id, caption: '' }]); }
  };
    ...
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border-2 border-amber-300/40">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                  ...
                  return (
                    <div key={s.paperId} className="flex gap-2 items-start p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
                      <div className="flex-1 min-w-0">
                        ...
                        <input value={s.caption} onChange={e => updateCaption(s.paperId, e.target.value)}
                          placeholder="Optional: Why is this featured? (shown as a note to users)"
                          className="w-full text-xs px-2.5 py-1.5 border border-amber-300 dark:border-amber-700 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-amber-500" />
                      </div>
          ...
          <div>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search papers..."
              className="w-full px-3 py-2 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm mb-2 focus:outline-none focus:border-navy dark:bg-gray-700 dark:text-white" />
            <div className="space-y-1.5 max-h-52 overflow-y-auto">
              {filtered.map(p => (
                <div key={p._id} onClick={() => toggle(p)}
                  className={`flex items-center gap-2 p-2.5 rounded-lg cursor-pointer transition ${isSelected(p._id) ? 'bg-navy/10 dark:bg-navy/20 border border-navy/30' : 'hover:bg-gray-50 dark:hover:bg-gray-700 border border-transparent'}`}>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
          ...
          <button onClick={() => { onSave(selected); onClose(); }} className="flex-1 px-4 py-2.5 bg-navy dark:bg-blue-600 text-white rounded-xl font-bold text-sm hover:opacity-90 transition flex items-center justify-center gap-2">
            <Save size={15} />Save Featured
          </button>
        </div>
      </div>
    </div>
...
const PaperCard = memo(({ paper, onClick, highlight, viewMode }) => {
  ...
  const hl = (text) => {
    ...
    return text.split(regex).map((part, i) =>
      regex.test(part) ? <mark key={i} className="bg-yellow-200 dark:bg-yellow-800/60 text-gray-900 dark:text-yellow-100 rounded px-0.5">{part}</mark> : part
    );
  };
  if (viewMode === 'list') {
    return (
      <div onClick={() => onClick(paper._id)} className="group bg-white dark:bg-gray-800/80 rounded-xl border-l-4 border border-gray-200 dark:border-gray-700/60 p-3 cursor-pointer hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5"
  ...
  return (
    <div onClick={() => onClick(paper._id)} className="group bg-white dark:bg-gray-800/80 rounded-xl border border-gray-200 dark:border-gray-700/60 p-4 cursor-pointer hover:shadow-xl transition-all duration-200 hover:-translate-y-1 flex flex-col overflow-hidden relative"
      style={{ borderLeft: `4px solid ${colors.accent}` }}>
      <div className="absolute top-0 right-0 w-20 h-20 opacity-5 rounded-bl-full" style={{ background: colors.accent }} />
      ...
      {paper.awards?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2.5">{paper.awards.slice(0, 3).map((a, i) => <AwardBadge key={i} award={a} />)}{paper.awards.length > 3 && <span className="text-xs text-gray-500 self-center">+{paper.awards.length - 3}</span>}</div>
      )}
...
// ─── Section Divider ──────────────────────────────────────────────────────────
const SectionDivider = ({ label, icon: Icon, color = 'text-gray-500', accent }) => (
...
const Explore = () => {
  ...
  // Resolve featured paper objects from IDs
  ...
  const sortOptions = [
    ...
    { value: 'awards-desc', label: 'Most Awards' }, { value: 'year-desc', label: 'Year ↓' },
    ...
    <>
      ...
      <div className="min-h-screen pb-8 bg-gray-50 dark:bg-gray-950">
        ...
        {(resolvedFeatured.length > 0 || isAdmin) && (
          <div className="mb-8 bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
        ...
        <div className="mb-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          {/* Panel Header */}
          <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
            <Search size={16} className="text-navy dark:text-accent" />
            <span className="font-bold text-sm text-gray-800 dark:text-gray-200">Search & Filter</span>
            {activeCount > 0 && (
              <span className="ml-auto text-xs bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 font-bold px-2 py-0.5 rounded-full">{activeCount} active</span>
            )}
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setSearchMode('simple')}
                className={`px-3 py-2.5 rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 ${searchMode === 'simple' ? 'bg-navy text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
                <Search size={15} />Simple Search
              </button>
              <button onClick={() => setSearchMode('advanced')}
            ...
            <form onSubmit={e => { e.preventDefault(); performSearch(); }}>
              <div className="relative mb-3">
                ...
                {showSuggestions && suggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border-2 border-navy/20 dark:border-accent/30 rounded-xl shadow-2xl z-20 overflow-hidden">
                    {suggestions.map((s, i) => (
                      <button key={i} type="button" onClick={() => { setQuery(s.text); setShowSuggestions(false); setTimeout(performSearch, 100); }}
                        className="w-full text-left px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center gap-2.5 transition">
              ...
              {/* Advanced — Semantic Search toggle with tooltip */}
              {searchMode === 'advanced' && (
                <label className="flex items-center gap-2.5 p-3 bg-purple-50 dark:bg-purple-900/30 rounded-xl border border-purple-200 dark:border-purple-700 cursor-pointer mb-3">
                  ...
                  <Tooltip position="bottom" content={
                    <div className="space-y-1.5">
                      <p className="font-bold text-white text-xs border-b border-gray-700 pb-1.5 mb-1.5 flex items-center gap-1.5"><Cpu size={11} />AI Semantic Search</p>
                      <p className="text-gray-300 text-xs leading-relaxed">Unlike regular keyword search, semantic search understands the <strong className="text-white">meaning</strong> behind your query.</p>
                      <p className="text-gray-300 text-xs leading-relaxed">For example, searching <em className="text-purple-300">"heart problems in elderly"</em> will also find papers about <em className="text-purple-300">"cardiac conditions in geriatric patients"</em> — even if those exact words don't appear.</p>
                      <p className="text-yellow-300 text-xs mt-1">⚡ Uses TF-IDF algorithm to rank results by conceptual relevance.</p>
                    </div>
                  }>
                    <div className="flex items-center gap-1 px-2 py-0.5 bg-purple-100 dark:bg-purple-800/50 rounded-full text-purple-600 dark:text-purple-300 text-xs font-semibold cursor-help hover:bg-purple-200 dark:hover:bg-purple-800 transition">
                      <HelpCircle size={11} />What's this?
                    </div>
                  </Tooltip>
                </label>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="submit" disabled={loading}
                  className="py-2.5 bg-navy hover:bg-navy-800 text-white rounded-xl font-bold text-sm disabled:opacity-50 flex items-center justify-center gap-2 transition shadow-md">
                ...
                <button type="button" onClick={() => setShowFilters(f => !f)}
                  className={`py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition border-2 ${showFilters ? 'bg-navy text-white border-navy shadow-md' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>
                  <SlidersHorizontal size={16} />Filters
                  {activeCount > 0 && <span className="bg-red-500 text-white text-xs font-black px-1.5 py-0.5 rounded-full leading-none">{activeCount}</span>}
                </button>
              </div>
            </form>
            ...
            {showFilters && (
              <div className="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-2 animate-fade-in">
                <select value={filters.category} onChange={e => setFilters(f => ({ ...f, category: e.target.value }))}
                  className="w-full px-3 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:border-navy focus:outline-none bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-gray-100">
                  <option value="">All Categories</option>
                  <option value="Completed">Completed</option>
                  <option value="Published">Published</option>
                </select>
                <select value={filters.yearCompleted} onChange={e => setFilters(f => ({ ...f, yearCompleted: e.target.value }))}
                  className="w-full px-3 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:border-navy focus:outline-none bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-gray-100">
                  <option value="">All Years</option>
                  {years.map(y => <option key={y} value={y}>{y}</option>)}
                </select>
                <select value={filters.subjectArea} onChange={e => setFilters(f => ({ ...f, subjectArea: e.target.value }))}
                  className="w-full px-3 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:border-navy focus:outline-none bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-gray-100">
                  <option value="">All Subjects</option>
                  {subjects.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <input type="text" value={filters.author} onChange={e => setFilters(f => ({ ...f, author: e.target.value }))}
                  placeholder="Filter by author name…"
                  className="w-full px-3 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:border-navy focus:outline-none bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400" />
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <button type="button" onClick={clearAll} className="py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl font-bold text-sm flex items-center justify-center gap-1.5 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <X size={14} />Clear All
                  </button>
                  <button type="button" onClick={() => { performSearch(); setShowFilters(false); }} className="py-2 bg-navy hover:bg-navy-800 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-1.5 transition">
            ...
            {/* Boolean search hint */}
            {searchMode === 'advanced' && (
              <div className="px-3 py-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">
                  <strong>Boolean tips:</strong> Use <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">AND</code>, <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">OR</code>, <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">NOT</code> · Field search: <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">author:Reyes</code>, <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">year:2024</code>
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-gray-800 dark:text-gray-200">
                <strong className="text-navy dark:text-accent">{sortedPapers.length}</strong> Papers
              </span>
              {activeCount > 0 && (
                <button onClick={clearAll} className="text-xs text-red-500 hover:text-red-600 font-bold inline-flex items-center gap-0.5 ml-1">
            ...
            <div className="flex items-center gap-2">
              <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                className="text-xs border-2 border-gray-300 dark:border-gray-600 rounded-xl px-2.5 py-1.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-semibold focus:border-navy focus:outline-none">
              ...
              <div className="flex gap-1 bg-gray-100 dark:bg-gray-700/50 border border-gray-300 dark:border-gray-600 p-1 rounded-xl">
                <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-lg transition ${viewMode === 'grid' ? 'bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-600' : ''}`}>
                  <Grid size={16} className={viewMode === 'grid' ? 'text-navy dark:text-accent' : 'text-gray-500'} />
                </button>
                <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-lg transition ${viewMode === 'list' ? 'bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-600' : ''}`}>
          ...
          {subjects.length > 0 && (
            <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
              <p className="text-xs text-gray-400 dark:text-gray-500 font-semibold mb-2 uppercase tracking-wider">Subject Legend</p>
              <div className="flex flex-wrap gap-1.5">
                {subjects.slice(0, 8).map(s => {
                  const c = getSubjectColor(s);
                  return (
                    <button key={s} onClick={() => { setFilters(f => ({ ...f, subjectArea: filters.subjectArea === s ? '' : s })); }}
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border transition hover:scale-105 ${c.bg} ${c.border} ${c.text} ${filters.subjectArea === s ? 'ring-2 ring-offset-1' : ''}`}
                      style={{ ringColor: c.accent }}>
            ...
            {loading ? (
              <div className="flex items-center justify-center py-16">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500 font-semibold">Searching…</p>
                </div>
              </div>
              ...
              <>
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
                  <Pagination
                    ...
                    itemsPerPage={itemsPerPage} onItemsPerPageChange={setItemsPerPage} totalItems={sortedPapers.length}
                  />
                </div>
              </>
            )}
      ...
      {showFeaturedManager && isAdmin && (
        <FeaturedPapersManager
          allPapers={allPapers}
          featuredList={featuredPapers}
