...
import { X, RotateCcw, Trash2, Clock, FileText, AlertTriangle, RefreshCw, ShieldAlert } from 'lucide-react';

// ── Custom Purge Confirm Modal ──
const PurgeConfirmModal = ({ isOpen, onClose, onConfirm, paperTitle }) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[300] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full border-2 border-red-600 animate-scale-in">
        <div className="bg-gradient-to-r from-red-700 to-red-600 p-5 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0">
              <ShieldAlert size={20} className="text-white" />
            </div>
            <div>
              <p className="font-bold text-white">Permanently Delete?</p>
        ...
        <div className="p-5 space-y-4">
          {paperTitle && (
            <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 p-3 rounded-lg text-sm text-red-800 dark:text-red-300 line-clamp-3">"{paperTitle}"</div>
          )}
          <div className="flex items-start gap-2 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-700 rounded-xl text-xs text-orange-800 dark:text-orange-300">
            <AlertTriangle size={13} className="flex-shrink-0 mt-0.5" />
            <span>This paper and its PDF file will be <strong>permanently deleted</strong> from the system. There is no way to recover it.</span>
          </div>
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 px-4 py-2.5 border-2 border-gray-300 dark:border-gray-600 rounded-xl font-semibold text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition text-gray-700 dark:text-gray-300">Cancel</button>
            <button onClick={() => { onConfirm(); onClose(); }} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow-lg">
              <Trash2 size={14} /> Purge Forever
            </button>
          </div>
        </div>
      </div>
...
const RecentlyDeletedModal = ({ isOpen, onClose, onRestored }) => {
  ...
  const [purgeTarget, setPurgeTarget] = useState(null); // { _id, title }
  ...
  const handleRestore = async (paperId, title) => {
    ...
    try {
      ...
      if (res.ok) { showToast(`"${title.slice(0, 40)}..." restored`); setDeletedPapers(prev => prev.filter(p => p._id !== paperId)); onRestored?.(); }
  ...
  const handlePurgeConfirmed = async () => {
    ...
    finally { setPurging(null); setPurgeTarget(null); }
  };

  const getDaysLeft = (deletedAt) => Math.max(0, 30 - Math.floor((Date.now() - new Date(deletedAt)) / (1000 * 60 * 60 * 24)));
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col border border-gray-200 dark:border-gray-700">
          ...
          <div className="bg-gradient-to-r from-orange-500 to-red-500 p-5 rounded-t-2xl flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="text-lg font-bold text-white">Recently Deleted Papers</h2><p className="text-orange-100 text-xs">Auto-purged after 30 days · {deletedPapers.length} paper{deletedPapers.length !== 1 ? 's' : ''}</p></div>
            </div>
          ...
          {toast && (
            <div className={`mx-5 mt-3 flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 ${toast.type === 'error' ? 'bg-red-600 text-white' : 'bg-green-600 text-white'}`}>{toast.msg}</div>
          )}
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {deletedPapers.map(paper => {
                  ...
                  return (
                    <div key={paper._id} className={`bg-gray-50 dark:bg-gray-800 rounded-xl border-2 p-4 hover:shadow-md transition ${daysLeft <= 3 ? 'border-red-200 dark:border-red-800' : 'border-gray-200 dark:border-gray-700'}`}>
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 bg-red-100 dark:bg-red-900/30 rounded-lg flex items-center justify-center flex-shrink-0"><FileText size={18} className="text-red-600 dark:text-red-400" /></div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-sm text-gray-900 dark:text-white line-clamp-2 mb-1">{paper.title}</h4>
                          <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500 mb-2">
                            <span>{paper.authors?.join(', ')}</span>
                            <span>•</span><span>{paper.category}</span>
                            <span>•</span><span>Deleted {new Date(paper.deletedAt).toLocaleDateString()}</span>
                            {paper.deletedByName && <><span>•</span><span>by {paper.deletedByName}</span></>}
                          </div>
                          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${getDaysColor(daysLeft)}`}>
                            <Clock size={10} />{daysLeft === 0 ? 'Expires today' : `${daysLeft} day${daysLeft !== 1 ? 's' : ''} left`}
                          </div>
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          <button onClick={() => handleRestore(paper._id, paper.title)} disabled={restoring === paper._id || purging === paper._id}
                            ...
                            {restoring === paper._id ? <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <RotateCcw size={13} />}Restore
                          </button>
                          <button onClick={() => setPurgeTarget({ _id: paper._id, title: paper.title })} disabled={restoring === paper._id || purging === paper._id}
                            ...
                            {purging === paper._id ? <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Trash2 size={13} />}Purge
                          </button>
                        </div>
                      </div>
                    </div>
          ...
          <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
            <button onClick={onClose} className="w-full py-2.5 border-2 border-gray-300 dark:border-gray-600 rounded-xl text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition">Close</button>
          </div>
        </div>
      </div>
    </>
