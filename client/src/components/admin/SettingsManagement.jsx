...
import { Settings, Image, Upload, Save, AlertCircle } from 'lucide-react';
...
const SettingsManagement = () => {
  ...
  const handleLogoUpload = async (type, file) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(`${type} logo uploaded successfully`);
        fetchSettings();
  ...
  if (loading) {
    return <div className="flex justify-center p-8"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-navy"></div></div>;
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {['school', 'college', 'conserve'].map((type) => (
            <div key={type} className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 text-center">
              ...
              {settings?.logos?.[type]?.url ? (
                <div className="mb-4">
                  <img 
                    ...
                    alt={`${type} logo`}
                    className="w-24 h-24 object-contain mx-auto mb-2 rounded-lg bg-gray-100 dark:bg-gray-700 p-2"
                  />
              ...
              <label className="cursor-pointer inline-flex items-center gap-2 bg-navy text-white px-4 py-2 rounded-lg hover:bg-navy-800 transition text-sm">
                <Upload size={16} />
                Upload New
                <input
                  type="file"
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-blue-700 dark:text-blue-400">
              ...
              <ul className="list-disc list-inside space-y-1">
                ...
                <li>Accepted formats: PNG, JPG</li>
          ...
          {[
            ...
            <label key={feature.key} className="flex items-start gap-3 p-4 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition">
              <input
                ...
                className="w-5 h-5 text-navy border-gray-300 rounded focus:ring-navy mt-0.5"
              />
