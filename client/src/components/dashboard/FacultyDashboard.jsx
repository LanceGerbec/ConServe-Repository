...
import { FileText, Clock, Eye, BookOpen, Activity, Bookmark, Calendar, Users, Upload, Search, X, ChevronRight, Filter, Info, Trash2, Edit2 } from 'lucide-react';
...
const FacultyDashboard = () => {
  ...
  const getReviewBadge = (paper) => {
    if (paper.reviewedByCurrentUser) {
      return (
        <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 rounded-lg text-xs font-bold whitespace-nowrap">
          ✓ REVIEWED
        </span>
      );
    }
    ...
    if (paper.totalReviewsCount > 0) {
      return (
        <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 rounded-lg text-xs font-bold whitespace-nowrap">
          {paper.totalReviewsCount} REVIEW{paper.totalReviewsCount !== 1 ? 'S' : ''}
        </span>
      );
    }

    return (
      <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 rounded-lg text-xs font-bold whitespace-nowrap">
        PENDING
      </span>
    );
  };
  ...
  const filteredReviews = myReviews.filter(r =>
    r.research?.title?.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3 mb-3">
        <div className="flex-1 min-w-0 w-full sm:w-auto">
          <h3
            ...
            onClick={() => window.location.href = `/research/${isBookmark ? paper.research._id : isReview ? paper.research._id : paper._id}`}
          >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        ...
        <div className="w-full sm:w-auto flex gap-2">
          {isBookmark ? (
            ...
            >
              <Eye size={14} />
              {paper.reviewedByCurrentUser ? 'View' : 'Review'}
            </button>
          ) : (
            <button
              onClick={() => window.location.href = `/research/${paper.research._id}`}
                  ...
                  <div className="flex flex-col gap-3 mb-4">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-9 py-2.5 sm:py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-navy focus:ring-4 focus:ring-navy/10 focus:outline-none dark:bg-gray-900"
                      />
                      {search && (
                        <button
                          onClick={() => setSearch('')}
                  ...
                  {filteredReviews.length === 0 ? (
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {filteredReviews.map(review => (
                        <PaperCard key={review._id} paper={review} isReview />
                      ))}
                    </div>
                  )}
                </div>
          ...
          {activeTab === 'submissions' && (
            <div ref={submissionsRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden scroll-mt-4">
              <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
                ...
                <button
                  onClick={() => setShowIMRaDModal(true)}
                  className="w-full flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 bg-navy text-white rounded-xl font-semibold shadow-lg active:scale-95 transition-all text-sm"
                >
                  <Upload size={16} />
                  Submit Research
                </button>
            ...
            <div ref={bookmarksRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden scroll-mt-4">
              <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip content={`${bookmarks.length} bookmarked paper${bookmarks.length !== 1 ? 's' : ''}`} position="bottom">
                    <div className="flex items-center gap-1 text-navy dark:text-accent cursor-help">
