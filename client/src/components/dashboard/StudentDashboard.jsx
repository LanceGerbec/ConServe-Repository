...
// CHANGE: handleDeleteRejected now uses PasswordDeleteModal with /author-delete-with-password endpoint
// All other code is identical to original
...
const StudentDashboard = () => {
  ...
  // CHANGED: password delete modal instead of simple confirm
  const [passwordDeleteModal, setPasswordDeleteModal] = useState({ isOpen: false, paperId: null, title: '' });
  ...
  const handleEdit = (paper) => { setEditingPaper(paper); setShowEditModal(true); };

  // CHANGED: Open password modal instead of basic confirm
  const handleDeleteRejected = (paperId, title) => {
    setPasswordDeleteModal({ isOpen: true, paperId, title });
  };

  // NEW: Password-confirmed delete
  const confirmPasswordDelete = async (password) => {
    const { paperId } = passwordDeleteModal;
    const token = localStorage.getItem('token');
    const res = await fetch(`${API_URL}/research/${paperId}/author-delete-with-password`, {
      method: 'POST',
    ...
    setPasswordDeleteModal({ isOpen: false, paperId: null, title: '' });
    showToast('Paper moved to recently deleted', 'success');
  };
  ...
  const StatCard = ({ icon: Icon, label, value, color, onClick }) => (
    <div onClick={onClick} className={`bg-white dark:bg-gray-800 rounded-xl p-3 sm:p-4 shadow-md border-2 border-gray-100 dark:border-gray-700 transition-all ${onClick ? 'active:scale-95 cursor-pointer hover:shadow-lg' : ''}`}>
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={`w-10 h-10 sm:w-12 sm:h-12 ${color} rounded-xl flex items-center justify-center shadow-md flex-shrink-0`}><Icon className="text-white" size={18} /></div>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-5 shadow-md border border-gray-200 dark:border-gray-700 active:scale-98 transition-all">
      <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3 mb-3">
        <div className="flex-1 min-w-0 w-full sm:w-auto">
          ...
          <div className="mb-2 flex items-start gap-2">
            ...
            <p className="text-xs text-gray-700 dark:text-gray-300 line-clamp-2">{isBookmark ? paper.research.authors?.join(', ') : paper.authors?.join(', ')}</p>
          </div>
        </div>
        {isSubmission && <span className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs font-bold whitespace-nowrap self-start ${getStatusBadge(paper.status)}`}>{paper.status?.toUpperCase()}</span>}
      </div>
      <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-2 leading-relaxed">{isBookmark ? paper.research.abstract : paper.abstract}</p>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div className="flex flex-wrap gap-2 sm:gap-3 text-xs text-gray-500 dark:text-gray-400">
          <span className="flex items-center gap-1.5"><Calendar size={14} /><span>{new Date(paper.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span></span>
          ...
          {isBookmark ? (
            ...
            <>
              {canEdit(paper.status) && (
                <button onClick={() => handleEdit(paper)} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg text-xs font-bold transition active:scale-95"><Edit2 size={14} />Edit</button>
              )}
              {canDelete(paper.status) && (
                <button onClick={() => handleDeleteRejected(paper._id, paper.title)} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-xs font-bold transition active:scale-95"><Trash2 size={14} />Delete</button>
              )}
            </>
          )}
        </div>
  ...
  return (
    <>
      ...
      <PasswordDeleteModal
        ...
        onClose={() => setPasswordDeleteModal({ isOpen: false, paperId: null, title: '' })}
        onConfirm={confirmPasswordDelete}
        itemName={passwordDeleteModal.title}
        message="This will move the paper to recently deleted. It can be restored within 30 days."
      />
        ...
        <div className="bg-gradient-to-r from-[#1e3a8a] via-[#1e40af] to-[#2563eb] p-4 sm:p-6 mb-4 sm:mb-6 shadow-xl relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-3 sm:gap-6">
            <div className="hidden xs:block flex-shrink-0">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-400 via-blue-500 to-blue-600 flex items-center justify-center shadow-2xl ring-2 sm:ring-4 ring-white/20">
                <span className="text-lg sm:text-2xl font-bold text-white tracking-tight">{user?.firstName?.charAt(0)}{user?.lastName?.charAt(0)}</span>
              </div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 sm:gap-6 text-xs sm:text-sm flex-wrap">
                <div className="flex items-center gap-1.5 sm:gap-2 text-blue-100"><Upload size={14} className="text-blue-300 flex-shrink-0" /><span className="font-semibold text-white">{stats.submissions}</span><span className="text-blue-200 hidden sm:inline">Submission{stats.submissions !== 1 ? 's' : ''}</span><span className="text-blue-200 sm:hidden">Paper{stats.submissions !== 1 ? 's' : ''}</span></div>
                <div className="w-px h-3 sm:h-4 bg-blue-400/30"></div>
                <div className="flex items-center gap-1.5 sm:gap-2 text-blue-100"><Eye size={14} className="text-blue-300 flex-shrink-0" /><span className="font-semibold text-white">{stats.views}</span><span className="text-blue-200">Views</span></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {[
              ...
              <button key={tab.id} onClick={() => {
                ...
                <tab.icon size={16} />
                <span>{tab.label}</span>
                {tab.badge > 0 && <span className="ml-0.5 sm:ml-1 px-1.5 sm:px-2 py-0.5 bg-[#FFB27F] text-white text-xs font-bold rounded-full">{tab.badge}</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="px-3 sm:px-4 space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 gap-3 sm:gap-4">
              <button onClick={() => setShowIMRaDModal(true)} className="bg-gradient-to-br from-navy to-blue-700 text-white p-4 sm:p-6 rounded-2xl shadow-lg active:scale-95 transition-all">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white/20 rounded-xl flex items-center justify-center"><Upload size={20} /></div>
                  <div className="text-left flex-1"><h3 className="font-bold text-base sm:text-lg mb-0.5 sm:mb-1">Submit Research</h3><p className="text-xs sm:text-sm text-blue-100 opacity-90">Upload your paper</p></div>
              ...
              <button onClick={() => window.location.href = '/explore'} className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg border-2 border-gray-200 dark:border-gray-700 active:scale-95 transition-all">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center"><BookOpen className="text-blue-600" size={20} /></div>
                  <div className="text-left flex-1"><h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-0.5 sm:mb-1">Browse Papers</h3><p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Explore repository</p></div>
          ...
          {activeTab === 'submissions' && (
            <div ref={submissionsRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden scroll-mt-4">
              <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-3 sm:mb-4 flex items-center gap-2"><Upload size={18} className="text-blue-600" />My Submissions ({filteredSubmissions.length})</h2>
                <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search submissions..." className="w-full pl-9 sm:pl-10 pr-9 sm:pr-10 py-2.5 sm:py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:border-navy focus:ring-4 focus:ring-navy/10 focus:outline-none dark:bg-gray-900" />{search && <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"><X size={16} /></button>}</div>
              </div>
              <div className="p-3 sm:p-4">
                {filteredSubmissions.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4"><Upload size={28} className="text-gray-400" /></div>
          ...
          {activeTab === 'bookmarks' && (
            <div ref={bookmarksRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden scroll-mt-4">
              <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-3 sm:mb-4 flex items-center gap-2"><Bookmark size={18} className="text-purple-600" />Bookmarked Papers ({filteredBookmarks.length})</h2>
              ...
              <div className="p-3 sm:p-4">
                {filteredBookmarks.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4"><Bookmark size={28} className="text-gray-400" /></div>
      ...
      {showEditModal && editingPaper && <EditResearch research={editingPaper} onClose={() => { setShowEditModal(false); setEditingPaper(null); }} onSuccess={() => { setShowEditModal(false); setEditingPaper(null); fetchData(); }} />}
