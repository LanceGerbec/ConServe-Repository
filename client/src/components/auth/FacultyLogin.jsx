...
import { Eye, EyeOff, Users, Loader2, Mail, Lock, X, Home, Clock } from 'lucide-react';
...
const LockoutTimer = ({ seconds, onExpire }) => {
  ...
  return (
    <div className="flex items-center gap-2 bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 p-3 rounded-lg mb-4">
    ...
    <>
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800">
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md p-10 border border-gray-200 dark:border-gray-800 animate-scale-in relative">
          ...
          {lockoutSeconds > 0 && <LockoutTimer seconds={lockoutSeconds} onExpire={() => setLockoutSeconds(0)} />}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" required value={formData.email}
                  ...
                  className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 dark:border-gray-700 rounded-xl focus:border-blue-500 focus:outline-none bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                  placeholder="faculty@neust.edu.ph" disabled={lockoutSeconds > 0} />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} required value={formData.password}
                  ...
                  className="w-full pl-12 pr-12 py-3 border-2 border-gray-300 dark:border-gray-700 rounded-xl focus:border-blue-500 focus:outline-none bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
            ...
            <button type="submit" disabled={loading || lockoutSeconds > 0}
              className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center space-x-2 disabled:opacity-50 shadow-lg">
              {loading ? <><Loader2 className="animate-spin" size={20} /><span>Signing in...</span></> : <><Users size={20} /><span>Sign In</span></>}
            </button>
          </form>

          <div className="mt-8 text-center">
            <Link to="/login" className="text-sm text-gray-600 dark:text-gray-400 hover:text-blue-500">← Back to role selection</Link>
          </div>
        </div>
      </div>
