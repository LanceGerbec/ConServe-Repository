    ...
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-950">
      ...
      <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            {user ? (
              <>
                {/* ✅ Use NotificationBell component (has dropdown built-in) */}
                <NotificationBell />
                  ...
                  <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                    ...
                    <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0">
                      {user?.avatar
                        ...
                        : <div className="w-full h-full bg-gradient-to-br from-navy to-accent flex items-center justify-center"><span className="text-white text-xs font-black">{initials}</span></div>}
                    </div>
                    ...
                    <div className="absolute right-0 top-full mt-1.5 w-56 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden animate-fade-in">
                      <div className="p-3 bg-gradient-to-r from-navy/5 to-accent/5 dark:from-navy/20 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 border-2 border-gray-200 dark:border-gray-600">
                          {user?.avatar
                            ? <img src={user.avatar} alt="avatar" className="w-full h-full object-cover" />
                            : <div className="w-full h-full bg-gradient-to-br from-navy to-accent flex items-center justify-center"><span className="text-white text-sm font-black">{initials}</span></div>}
                        </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 py-3 space-y-1 animate-fade-in">
            ...
            {user && (
              <>
                ...
                <Link to="/notifications"
                  className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold transition ${isActive('/notifications') ? 'bg-navy text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
                  Notifications
                </Link>
            ...
            {!user && (
              <Link to="/login" className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold bg-navy text-white">Sign In</Link>
            )}
      ...
      <footer className="relative mt-auto">
        <div className="overflow-hidden relative">
          <svg viewBox="0 0 1440 90" className="w-[200%] h-[70px] block wave-container -mb-[2px]" preserveAspectRatio="none">
            <defs>
            ...
            <path d="M0,60 C240,30 480,90 720,60 C960,30 1200,90 1440,60 L1440,90 L0,90 Z M1440,60 C1680,30 1920,90 2160,60 C2400,30 2640,90 2880,60 L2880,90 L1440,90 Z" fill="#020c1b" className="wave wave-back" />
            <path d="M0,65 C240,35 480,95 720,65 C960,35 1200,95 1440,65 L1440,90 L0,90 Z M1440,65 C1680,35 1920,95 2160,65 C2400,35 2640,95 2880,65 L2880,90 L1440,90 Z" fill="#03142b" className="wave wave-mid" />
            <path d="M0,70 C240,40 480,100 720,70 C960,40 1200,100 1440,70 L1440,90 L0,90 Z M1440,70 C1680,40 1920,100 2160,70 C2400,40 2640,100 2880,70 L2880,90 L1440,90 Z" fill="url(#waveGradient)" className="wave wave-front" />
          </svg>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
              <div>
                <div className="flex items-center gap-2.5 mb-4">
                  <a href="https://neust.edu.ph/" target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border-2 border-white/20 overflow-hidden bg-white hover:scale-110 transition flex-shrink-0">
                  ...
                  <a href="https://www.facebook.com/NEUSTCON" target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border-2 border-white/20 overflow-hidden bg-white hover:scale-110 transition flex-shrink-0">
                  ...
                  <Link to="/" className="w-9 h-9 rounded-lg bg-white/10 border border-white/20 overflow-hidden flex items-center justify-center flex-shrink-0 hover:scale-110 transition">
                  ...
                  <span className="font-black text-xl"><span className="text-blue-400">CON</span><span className="text-white">serve</span></span>
                </div>
                <p className="text-sm text-blue-300/70 leading-relaxed">NEUST College of Nursing<br />Research Repository</p>
              </div>
              <div>
                ...
                <div className="space-y-2">
                  {[['/', 'Home'], ['/about', 'About Us'], ['/help', 'Help Center'], ['/terms', 'Terms & Conditions'], ['/privacy', 'Privacy Policy']].map(([to, label]) => (
                    <Link key={to} to={to} className="block text-sm text-blue-300/70 hover:text-white transition font-medium">{label}</Link>
                  ))}
                </div>
              </div>
              <div>
                ...
                <Link to="/help" className="inline-block mt-4 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-xs font-bold text-white transition">Get Help →</Link>
              </div>
            </div>
