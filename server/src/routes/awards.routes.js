...
router.patch('/:id/awards/add', auth, authorize('admin', 'ret'), async (req, res) => {
  try {
    ...
    await AuditLog.create({
      user: req.user._id, action: 'AWARD_ADDED', resource: 'Research', resourceId: research._id,
      ipAddress: req.ip, userAgent: req.get('user-agent'), details: { awardName: name, color }
    });
...
router.delete('/:id/awards/remove', auth, authorize('admin', 'ret'), async (req, res) => {
  try {
    const { name } = req.body;
    ...
    await research.save();
    await AuditLog.create({
      user: req.user._id, action: 'AWARD_REMOVED', resource: 'Research', resourceId: research._id,
      ipAddress: req.ip, userAgent: req.get('user-agent'), details: { awardName: name }
    });
