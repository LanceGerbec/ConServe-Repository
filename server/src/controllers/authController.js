...
const fmtIp = (ip) => (ip || 'Unknown').replace('::ffff:', '');
  ...
  try {
    ...
    if (!isMatch) {
      ...
      Notification.create({
        ...
        message: `Failed login attempt detected from IP ${ip}. Account locked for ${attemptsAfter >= 5 ? '30min' : `${attemptsAfter}min`}.`,
        priority: attemptsAfter >= 3 ? 'high' : 'medium'
      }).catch(() => {});
    ...
    Notification.create({
      ...
      message: `Successful login from IP ${ip} on ${fmtDate()}.`,
      priority: 'low'
    }).catch(() => {});
...
export const forgotPassword = async (req, res) => {
  ...
  } catch { res.status(500).json({ error: 'Failed to process request' }); }
};
...
export const verifyResetToken = async (req, res) => {
  ...
  } catch { res.status(500).json({ valid: false, error: 'Verification failed' }); }
};
